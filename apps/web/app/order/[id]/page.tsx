...
export default function OrderPage() {
    ...
    async function startPolling() {
      ...
      if (data && !isFinalStatus(data.status)) {
        interval = setInterval(async () => {
          ...
          if (updatedOrder && isFinalStatus(updatedOrder.status)) {
            if (interval) {
  ...
  if (loading) {
    return <main className="mx-auto max-w-lg p-8">Loading order...</main>;
  }
  ...
  if (error || !order) {
    return (
      <main className="mx-auto max-w-lg p-8">
        ...
        <p className="mt-2 text-gray-600">We couldn't find this order.</p>
      </main>
    );
  }
    ...
    <main className="mx-auto max-w-lg p-8">
      ...
      <div className="mt-6 rounded-lg bg-gray-100 p-6">
        ...
        <p className="mt-2 text-sm text-gray-600">
          Keep your order number ready. The shop staff will verify your payment
